...
$sql = "SELECT id, enunciado, resolucao, ano FROM questoes WHERE concurso = 'FUVEST'"; // Filtro para concurso ENEM
  ...
  <div class="page-container">
    ...
    <main class="container mt-4">
      <div class="row">
        ...
        <div class="col-md-8">
          <?php
          // Alterando a consulta para pegar também o link do vídeo
$sql = "SELECT id, enunciado, resolucao, ano, video FROM questoes WHERE concurso = 'ENEM'"; // Agora inclui a coluna 'video'
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $id_questao = $row['id'];
        $ano_questao = $row['ano'];
        $video_url = $row['video']; // Obtendo o link do vídeo da coluna 'video'
        
        echo "<div class='questao' data-ano='$ano_questao' data-enunciado='" . strtolower($row['enunciado']) . "'>";
        ...
        // Verificando se há link de vídeo antes de exibir o botão
        if (!empty($video_url)) {
            ...
            echo "<h5>Vídeo:</h5>";
            echo "<a href='" . htmlspecialchars($video_url) . "' target='_blank'>Clique aqui para assistir ao vídeo</a>";
            echo "</div>";
        }
